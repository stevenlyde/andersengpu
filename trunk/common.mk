...
# Includes
INCLUDES  += -I. -I$(CUDA_INSTALL_PATH)/include -I$(SHAREDDIR)/inc
...
# Compiler-specific flags (by default, we always use sm_20), unless we use the SMVERSION template
...
# Libs
ifneq ($(DARWIN),)
    LIB       := -L$(CUDA_INSTALL_PATH)/lib -L$(LIBDIR) -L$(SHAREDDIR)/lib $(NVCUVIDLIB) 
else
  ifeq "$(strip $(HP_64))" ""
    ifeq ($(x86_64),1)
       LIB       := -L$(CUDA_INSTALL_PATH)/lib64 -L$(LIBDIR) -L$(SHAREDDIR)/lib 
    else
       LIB       := -L$(CUDA_INSTALL_PATH)/lib -L$(LIBDIR) -L$(SHAREDDIR)/lib
    endif
  else
    ifeq ($(i386),1)
       LIB       := -L$(CUDA_INSTALL_PATH)/lib -L$(LIBDIR) -L$(SHAREDDIR)/lib
...
# static linking, we will statically link against CUDA and CUDART
  ifeq ($(USEDRVAPI),1)
     ...
     LIB += ${OPENGLLIB} $(PARAMGLLIB) $(RENDERCHECKGLLIB) $(CUDPPLIB) 
  endif
endif
