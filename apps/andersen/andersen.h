...
// a few defines to avoid IDE errors when referencing threadIdx, blockIdx, etc.
#define threadId (__cudaGet_threadIdx())
#define blockId (__cudaGet_blockIdx())
#define blockDm (__cudaGet_blockDim())
#define gridDm (__cudaGet_gridDim())
#define warpSz (__cudaGet_warpSize())

/**
 * Compute warp index within block.
 * @return Number in range [0, THREADS_PER_BLOCK / WARP_SIZE)
 */
#define warpId (threadId.y)
/**
 * Compute thread index within warp.
 * @return Number in range [0..WARP_SIZE)
 */
#define threadWarpId (threadId.x)
